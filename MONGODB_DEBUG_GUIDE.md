...
- **MONGODB_DB** – Optional; defaults to `silicon-hubs` in `lib/db/mongodb.ts`.
...
client.connect().then(() => {
  return client.db(process.env.MONGODB_DB || 'silicon-hubs').collection('teamMembers').find({}).toArray();
...
| DB name           | `process.env.MONGODB_DB` or `silicon-hubs` |
...
3. **MongoDB Compass / mongosh:** Connect with the same `MONGODB_URI`, select DB `silicon-hubs` (or your `MONGODB_DB`), and check:
