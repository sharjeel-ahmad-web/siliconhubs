...
import { Timeline } from '@/modules/public/components/timeline-animation';
            ...
            {sectionContent.split('\n\n').map((paragraph, idx) => {
              ...
              if (paragraph.startsWith('### ')) {
                return (
                  <h4
                    key={idx}
                    className="mb-2 mt-4 text-lg font-semibold text-[#06b6d4]"
                  >
                    {paragraph.slice(4)}
                  </h4>
                );
                  ...
                  <ul key={idx} className="ml-4 space-y-2">
                    {paragraph.split('\n').map((item, i) => (
                      <li key={i} className="flex items-start gap-2">
                        <span className="mt-1 text-[#fc4c00]">✓</span>
      ...
      <section className="relative px-6 pb-20 pt-32">
        ...
        <div className="absolute inset-0 overflow-hidden">
          {blog.coverImage || blog.thumbnail ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-b from-[#06b6d4]/10 to-black" />
          )}
        </div>
        ...
        <div className="relative mx-auto max-w-4xl">
          ...
          >
            <Link
              href="/blog"
              className="mb-8 inline-flex items-center gap-2 text-slate-400 transition-colors hover:text-[#06b6d4]"
            >
          ...
          <motion.span
            ...
            className="mb-6 inline-block rounded-full bg-[#06b6d4] px-4 py-1 text-sm font-medium text-white"
          >
            {blog.category}
          </motion.span>
          ...
          >
            ...
            <button
              onClick={sharePost}
              className="flex items-center gap-2 transition-colors hover:text-[#06b6d4]"
            >
            ...
            >
              <div className="flex flex-wrap items-center gap-3">
                ...
                {blog.tags.map((tag) => (
                  <span
                    key={tag}
                    className="cursor-pointer rounded-full bg-[#1E293B] px-3 py-1 text-sm text-slate-300 transition-colors hover:bg-[#06b6d4]/20 hover:text-[#06b6d4]"
                  >
                    {tag}
                  </span>
                ))}
      ...
      {relatedBlogs.length > 0 && (
        <section className="bg-[#0F172A] px-6 py-20">
          <div className="mx-auto max-w-7xl">
            ...
            >
              Related <span className="text-[#06b6d4]">Posts</span>
            </motion.h2>
              ...
              {relatedBlogs.map((relatedBlog, index) => (
                ...
                >
                  <Link href={relatedBlog?.slug ? `/blog/${relatedBlog.slug}` : '/blog'}>
                    <div className="group overflow-hidden rounded-xl border border-slate-800 bg-[#1E293B]/50 transition-all duration-300 hover:border-[#06b6d4]/50">
                      ...
                      <div className="p-5">
                        <h3 className="line-clamp-2 text-lg font-semibold text-white transition-colors group-hover:text-[#06b6d4]">
                          {relatedBlog.title}
                        </h3>
