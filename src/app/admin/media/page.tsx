...
export default function MediaLibraryPage() {
  ...
  const handleUpload = async (fileList: FileList) => {
    ...
    const uploadPromises = Array.from(fileList).map(async (file) => {
      ...
      formData.append('folder', currentFolder || 'silicon-hubs');
    ...
    <div className="min-h-full">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <label className="flex cursor-pointer items-center gap-2 rounded-lg bg-[#06b6d4] px-4 py-2 text-white transition-colors hover:bg-[#06b6d4]/80">
      ...
      {notice && (
        ...
        >
          {notice.type === 'error' ? (
            <X
              className={`mt-0.5 h-4 w-4 ${notice.type === 'error' ? 'text-red-400' : 'text-green-400'}`}
            />
          ) : (
                ...
                <div className="grid grid-cols-6 gap-3">
                  {subfolders.map((folder) => (
                    <button
                      ...
                      className="group flex flex-col items-center gap-2 rounded-xl border border-slate-700/50 bg-[#1E293B] p-4 transition-all hover:border-[#06b6d4]/50 hover:bg-[#1E293B]/80"
                    >
