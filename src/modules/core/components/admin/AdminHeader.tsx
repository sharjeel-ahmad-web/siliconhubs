...
export default function AdminHeader({ user }: AdminHeaderProps) {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'contact':
        return <MessageSquare className="h-4 w-4 text-[#06b6d4]" />;
      case 'user':
        return <UserPlus className="h-4 w-4 text-green-400" />;
      case 'content':
        return <FileText className="h-4 w-4 text-[#fc4c00]" />;
      default:
        ...
        <div className="max-w-md flex-1">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-full rounded-lg border border-slate-700 bg-[#0F172A] py-2 pl-10 pr-4 text-white placeholder-slate-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#06b6d4]"
            />
          </div>
        </div>
            ...
            {showNotifications && (
              <div className="absolute right-0 z-50 mt-2 w-80 overflow-hidden rounded-xl border border-slate-700 bg-[#1E293B] shadow-xl">
                <div className="flex items-center justify-between border-b border-slate-700 p-4">
                  ...
                  {unreadCount > 0 && (
                    <button
                      ...
                      className="text-xs text-[#06b6d4] transition-colors hover:text-[#06b6d4]/80"
                    >
                      Mark all as read
                    </button>
                  )}
                ...
                <div className="border-t border-slate-700 p-3">
                  <button className="w-full py-2 text-center text-sm text-[#06b6d4] transition-colors hover:text-[#06b6d4]/80">
