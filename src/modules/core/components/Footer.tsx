...
export default function Footer() {
  ...
  const copyrightText = (
    ...
    '© {year} Silicon Hubs. All rights reserved.'
  ).replace('{year}', new Date().getFullYear().toString());

  return (
    <footer className="border-t border-[#06b6d4]/20 bg-black">
      <div className="mx-auto max-w-7xl px-6 py-16">
                ...
                {column.links
                  ...
                  .map((link) => (
                    <li key={link.href}>
                      <Link
                        href={link.href ?? '#'}
                        className="font-inter text-sm text-[#64748B] transition-colors hover:text-[#06b6d4]"
                      >
                        {link.label}
                      </Link>
                    </li>
            ...
            <div>
              ...
              <form onSubmit={handleSubscribe} className="flex gap-2">
                <input
                  ...
                  required
                  className="flex-1 rounded-lg border border-[#06b6d4]/30 bg-[#1E293B] px-4 py-2 font-inter text-sm text-white focus:border-[#06b6d4] focus:outline-none"
                />
                <ParticleWrapper>
                  <button
                    type="submit"
                    disabled={submitting}
                    className="rounded-lg bg-[#fc4c00] px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-[#e0741d] disabled:opacity-50"
                  >
                    {submitting ? '...' : '→'}
                  </button>
                </ParticleWrapper>
        ...
        {/* Bottom bar */}
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-[#06b6d4]/20 pt-8 md:flex-row">
            ...
            <div className="flex items-center gap-3">
              {socialItems.map(({ key, data, label, Icon }) => (
                <a
                  ...
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-[#fc4c00]/50 bg-[#1E293B] text-[#fc4c00] transition-colors hover:border-[#06b6d4] hover:text-[#06b6d4]"
                  aria-label={label}
                >
